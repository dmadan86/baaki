...
let clock = 0;
/** A monotonic fake instant, so the overlay's clientCreatedAt ordering is stable. */
const tick = (): string => `2026-05-01T00:00:${String(clock++ % 60).padStart(2, '0')}.000Z`;
...
function env(
  ...
  return { clientMutationId: `mut-${clock++}`, kind, groupId, clientCreatedAt: tick(), payload };
}
  ...
  it('shows exactly the items created-and-not-removed, in a stable order', () => {
    fc.assert(
      fc.property(planOps(), (ops) => {
        // The model: which ids exist in THIS group and whether each is removed.
        ...
        for (const op of ops) {
          switch (op.t) {
            case 'create':
              // A create for an id already created is a duplicate the overlay drops (idempotency key).
              if (!live.has(op.id) && !removed.has(op.id)) {
                envs.push(
        ...
        // 4. Every visible row is genuinely live.
        for (const r of open) expect(r.deleted_at).toBeNull();
