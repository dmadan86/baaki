...
describe('email provider configuration', () => {
  it('defaults to Resend and asks for the Resend key', () => {
    ...
    expect(emailSendable()).toEqual({ ok: true });
  });

  it('selects SendGrid, but refuses to let it carry the retrying queues', () => {
    configure('sendgrid');

    expect(emailProvider()).toBe('sendgrid');
    expect(emailKeyName()).toBe('SENDGRID_API_KEY');
    // The adapter works — the tests below drive it — but until a send whose
    // outcome was never learned can be deduplicated, the queues that retry are
    // not allowed to use it. The refusal names the reason rather than looking
    // like email is switched off.
    expect(emailSendable()).toEqual({
      ok: false,
      reason: expect.stringContaining('idempotency key'),
    });
  });

  it('reports a deployment that has simply not turned email on, without a reason', () => {
    vi.stubEnv('EMAIL_PROVIDER', 'resend');

    expect(emailSendable()).toEqual({ ok: false, reason: '' });
  });
  ...
  it('refuses an unknown provider instead of silently treating email as off', () => {
    ...
    // Reported, not swallowed: a typo that reads as "email is off" is one
    // nobody finds until somebody asks why the mail stopped.
    expect(emailSendable()).toEqual({
      ok: false,
      reason: expect.stringContaining('sendgird'),
    });
  });
});
