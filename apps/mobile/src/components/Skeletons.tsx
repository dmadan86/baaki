...
/**
 * The friends tab list, mid-load.
 *
 * The loaded list is one bordered card of hairline-divided avatar rows under the
 * indigo hero — no section headings, no footnote (an earlier sectioned design
 * had both; this is shaped to the current one so the swap is a fill, not a
 * jump). The card fills the region so the shimmer covers the visible area rather
 * than sitting as a stub at the top. Shown under the hero while the balances
 * load — the hero itself is real, so only this area waits.
 */
export function PeopleSkeleton() {
  const theme = useTheme();
  // The loaded Friends list is one bordered card of hairline-divided avatar rows
  // (no section headings, no footnote). The skeleton is that same card, filling
  // the region (`flex: 1`) so the shimmer covers the visible area rather than a
  // stub at the top, and the swap to real rows is a fill, not a jump. Enough
  // rows to reach the bottom of a phone screen; the card border runs full height.
  return (
    <LoadingRegion style={{ flex: 1 }}>
      <View
        style={{
          flex: 1,
      ...
      >
        {Array.from({ length: 9 }, (_, index) => (
          <View key={index}>
            <SkeletonRow />
            {index < 8 ? <View style={{ height: 1, backgroundColor: theme.color.border }} /> : null}
          </View>
        ))}
      </View>
    </LoadingRegion>
