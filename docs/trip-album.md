...
(`waves_add_trip_photo` / `waves_remove_trip_photo`), the Prisma model, and the
...
| --------------- | --------------------------- | --------------- | ------------------------------------------------ | ------------------------------------ |
| Group **cover** | `groups.photo_path`         | one per group   | admin, **paid** (`waves_can_upload_group_photo`) | the group's identity                 |
...
- **Offline-first (ADR-005).** `updated_seq` + the shared `waves_stamp_seq`
...
- **RPC-only writes.** `REVOKE ALL … GRANT SELECT`; `waves_add_trip_photo` /
  `waves_remove_trip_photo` are the only way in, so `created_by` comes from the
...
- Bytes go to Cloudflare R2 under a new logical bucket `trip-photos`, brokered by
  ...
  free-tier storage cap exactly like a receipt (`waves_storage_reserve/record`).
...
`waves-adr.md` recording the three-photo-concepts split and the album's
